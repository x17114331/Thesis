...
public class Simulation extends BaseCloudSimObservable implements Constants {
	...
	public void runSimulation() throws Exception {
		...
		for (DatacenterController dcb : dcbs) {
			...
			for (Object o : res) {
				...
				m.setAvg(stat.average(measure));
                                avgproctime = avgproctime + stat.average(measure);
		...
		results.put(Constants.UB_STATS, ubResults);
                ...
                String faproc = new DecimalFormat("#.###").format(avgproctime);
                String fres = new DecimalFormat("#.###").format(restime);
                String fall = new DecimalFormat("#.###").format(alltotalcost);


                //String sumresultStr = loadBalancePolicy + "#" + faproc + "#" + fres + "#" + fall;

                //FileAppender.AppendtoFile("summary.txt",sumresultStr);
	...
	private void printVmAllocations(String dcName, Map<Integer, Integer> list){
		...
		for (Integer vm : list.keySet()){
			System.out.println(vm + "->" + list.get(vm));
                        ...
                        String ct = tag + "#" + vm + "#" + list.get(vm);
                        if (list.get(vm)>maxall)
                        {
                            maxall=list.get(vm);
                        }
                        ...
                        vmcost=vmcost + (list.get(vm)*100.0/maxall);
                        

		} 
                
                double mutil = maxall*100.0/tota;
                String ct = tag + "#" + tota + "#" + mutil;
                FileAppender.AppendtoFile("Perfg2.txt", ct);
                ct = tag + "#" + tota + "#" + vmcost;
                FileAppender.AppendtoFile("Perfg3.txt", ct);
