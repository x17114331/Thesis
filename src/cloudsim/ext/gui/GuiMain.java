...
private static final String CMD_ABOUT = "About";
	...
	private void initUI(){		
		this.setTitle("Load Based Cloud Paritioning");
	...
	private JPanel getMenuPanel(){
		...
		addMenuButton(menuPanel, CMD_EXIT);
                menuPanel.add(Box.createVerticalStrut(20));
                addMenuButton(menuPanel, "RESULT SUMMARY");
                //menuPanel.add(Box.createVerticalStrut(20));
                //addMenuButton(menuPanel, "CLEAR SUMMARY");

                //Bala added for commenting interner characteristics
                JButton btn = menuButtons.get(CMD_DEFINE_INTERNET_CHARACTERISTICS);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals(CMD_CONFIGURE_SIMULATION)){			
			...
			showHomeScreen();
		} else if (e.getActionCommand().equals("RESULT SUMMARY"))
                {
                     ResultSummary rs = new ResultSummary();
                     rs.setVisible(true);
                }
                else if (e.getActionCommand().equals("CLEAR SUMMARY"))
                {
                    /* try
                     {
                         File fh = new File("summary.txt");
                         fh.delete();
