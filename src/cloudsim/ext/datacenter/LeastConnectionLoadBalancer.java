...
/**
 * LeastConnectionLoadBalancer load balances the tasks among available VM's in a way to even out
 * the number of active tasks at any given time on each VM.
 * 
 * @author Bala
 *
 */
public class LeastConnectionLoadBalancer extends VmLoadBalancer implements CloudSimEventListener {
	...
	public LeastConnectionLoadBalancer(DatacenterController dcb){
		...
		if (currentAllocationCounts.size() < vmStatesList.size()){
			...
			for (int thisVmId : currentAllocationCounts.keySet()){
				...
				currCount = currentAllocationCounts.get(thisVmId);
                                //System.out.println(thisVmId + "==>" + currCount);
